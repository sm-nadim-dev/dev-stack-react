...
JSX is a syntax extension that lets me write HTML-like code inside JavaScript. It makes writing and reading UI components much easier and cleaner.
...
`useState` lets a component remember and update data over time. I used it in `ExploreTech.tsx` to track the user's selected `stack` items.

### 4. What does the useEffect hook do, and why did you need it to load the JSON data?
`useEffect` handles side effects like data fetching after rendering. However, I didn't use it here; I used React 19’s modern `Suspense` and `use()` promise architecture instead.
...
Conditional rendering displays different UI based on a condition. I used it in the sidebar to show either an "empty stack" message or the list of selected technologies.
